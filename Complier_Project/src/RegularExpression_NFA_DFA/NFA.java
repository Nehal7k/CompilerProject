package RegularExpression_NFA_DFA;
...
public class NFA {
	...
	public NFA(String re) {
		this.regExpression = re;
		reJoined = null;
		rePostfix = null;
		Set<Character> temp = new HashSet<>();
		for(int i=0;i<this.regExpression.length();i++){
			if(is_letter(this.regExpression.charAt(i))){
				temp.add(this.regExpression.charAt(i));
			}
		}
	...
	public String add_join_symbol() {
		...
		if(length==1) {
			System.out.println("add join symbol result:" + regExpression);
			reJoined = regExpression;
		...
		for (int i = 0; i < length - 1; i++) {
			first = regExpression.charAt(i);
			second = regExpression.charAt(i + 1);
		...
		System.out.println("add join symbol result :" + rString);
		System.out.println();
		reJoined = rString;
		return rString;
	}
	...
	public String print() {
		...
		revisit(this.pair.startNode);
                    ...
                    result +="start state: " + (this.pair.startNode.getState()) + "\n";
                    System.out.println("end state: " + (this.pair.endNode.getState()));
                    result +="end state: " + (this.pair.endNode.getState())+ "\n";
